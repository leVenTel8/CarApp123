...
namespace CarApp
{
    public partial class MainForm : Form
    {
        ...
        public static int Global_SelectRowMainForm;
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (dataGridViewMainForm.SelectedRows.Count > 0)
            {
                ...
                Global_SelectRowMainForm = id;             // глобальная переменная выбранной строки
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dataGridViewMainForm.SelectedRows.Count > 0)
            {
                ...
                //каскадное почему то не захотело работать, пришлось удалять так
                List<Inspection> dell = db.Inspections.Where(p => p.CarId == MainForm.Global_SelectRowMainForm).ToList();                       
                db.Inspections.RemoveRange(dell);
                db.Cars.Remove(car);
        ...
        private void btnEditInspection_Click(object sender, EventArgs e)
        {
            if (dataGridViewMainForm.SelectedRows.Count > 0)
            {
              ...
              InspectionTableForm frmInsTab = new InspectionTableForm();
              frmInsTab.Show();

        }

        //перехват выбранной строки на MainForm
        private void dataGridViewMainForm_MouseClick(object sender, MouseEventArgs e)
        {
            if (dataGridViewMainForm.SelectedRows.Count > 0)
            {
                int index = dataGridViewMainForm.SelectedRows[0].Index;
